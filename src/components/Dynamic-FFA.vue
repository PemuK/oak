    ...
    <el-card class="func-box">
      ...
      <div class="memory-container">
        ...
        >
          ...
          >
            <span class="pppp">{{ partition.occupiedBy.name }}占用</span>
          </div>
        </div>
      </div>
    </el-card>
...
// 动态切换 Pinia 状态
const dynamicMem = useDynamicMem();
...
const occupiedWidth = (partitionStart: number) => {
  ...
  if (partition && partition.occupiedBy) {
    return (partition.occupiedBy.memorySize / (partition.end - partition.start)) * 100;
  }
