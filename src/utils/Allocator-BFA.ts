import {useDynamicMem} from "@/stores/DynamicMem-BFA";
...
class BfaAllocator {
    ...
    constructor(type: number) {
        ...
        this.queue = useWorkerStoreBFA();
        watch(
            () => this.queue.activeQueue,  // 监视 activeQueue
            (newVal: Worker[], oldVal: Worker[]) => {
                // 你可以在这里执行你需要的操作
                const emptyPartitions = this.mem.methods.listEmpPartitionBySize();
    ...
    public async BFA(): Promise<boolean> {
        // 用来模拟任务分配的延迟
