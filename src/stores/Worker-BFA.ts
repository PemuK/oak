...
export const useWorkerStoreBFA = defineStore('worker', () => {
    ...
    function initWorkerQueue(): Worker[] {
        ...
        try {
            workQueueStorage = JSON.parse(localStorage.getItem('worker-queue-bnf') || '[]');
        } catch (e) {
            // 如果解析出错，则返回一个空数组
            workQueueStorage = [];
        }
        ...
        if (!workQueueStorage || workQueueStorage.length === 0) {
            ...
            localStorage.setItem('worker-queue-bnf', JSON.stringify(defaultQueue));
            return defaultQueue;
        }
    ...
    function initClogQueue(): Worker[] {
        ...
        // 尝试从 localStorage 获取工作队列数据
        try {
            const storedData = localStorage.getItem('clog-queue-bnf');
            if (storedData) {
    ...
    function addWorkerToQueue(newWorker: Worker) {
        ...
        localStorage.setItem('worker-queue-bnf', JSON.stringify(workQueue.value));
    }

    // 删除工作队列中的任务
    function removeFromWorkQueue(workerId: number) {
        workQueue.value = workQueue.value.filter(worker => worker.id !== workerId);
        localStorage.setItem('worker-queue-bnf', JSON.stringify(workQueue.value)); // 更新到 localStorage
    }
    ...
    function dequeueClogQueue(): Worker | undefined {
        ...
        if (worker) {
            localStorage.setItem('clog-queue-bnf', JSON.stringify(clogQueue.value));
        }
        return worker;
    }
