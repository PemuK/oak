    ...
    function generateRandomQueue(count: number): Worker[] {
        ...
        for (let i = 0; i < count; i++) {
            const newWorker: Worker = {
                id: Math.floor(Math.random() * 10000), // 随机生成一个ID
                name: `任务 ${Math.floor(Math.random() * 1000)}`, // 随机生成工作任务名称
                memorySize: Math.floor(Math.random() * 491) + 10, // 随机生成内存大小，范围 10-500
                runTime: Math.floor(Math.random() * 10) + 1, // 随机生成运行时间，范围 1-60
        ...
        workQueue.value = [...newQueue]; // 将生成的工作队列添加到现有队列中
        localStorage.setItem('worker-queue', JSON.stringify(workQueue.value)); // 更新到 localStorage
        return newQueue;
    }
